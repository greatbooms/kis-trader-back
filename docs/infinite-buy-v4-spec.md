...
- 주문가는 `현재가 × (1 + maxBuyPremiumPct)` 상한으로 clamp하고, clamp 발생 시 `details.v4BuyPriceClamps`에 phase별로 기록한다 (`maxBuyPremiumPct` 파라미터, 기본 0.10 — D11).
  - **첫 매수(`v4-first-buy`) 예외**: clamp 미적용. "사실상 무조건 체결" 의도(D7의 markup 12%)를 보존하기 위함 — 첫 매수까지 clamp하면 사이클 시작 자체가 봉쇄될 수 있다.
...
| D11 | 매수 지정가를 `현재가 × (1+maxBuyPremiumPct)`로 clamp, 기본 10%, 첫 매수는 예외 | KIS 실거래에서 평단 기준 사다리가가 급락 후 현재가와 괴리되며 "Too Aggressive" 거부가 반복 관측됨 (SOXL, 2026-08-04~05, 평단 대비 사다리가가 현재가보다 40%+ 높아 매일 거부). 정확한 KIS 허용 밴드 수치는 미확인 — 10%는 재발 방지용 잠정값이며, 밴드가 확인되면 근거 있는 수치로 교체한다 |
