...
**Architecture:** GitHub Actions builds a production Docker image, pushes it to GHCR, connects to the NAS through Tailscale SSH, uploads the Compose/deploy files, and runs `docker compose pull` plus `docker compose up -d`. Runtime secrets stay on the NAS in `.env.prod`; the container uses host networking so the app binds NAS port `20000` directly and reaches the NAS-hosted PostgreSQL port through `127.0.0.1`. After the container is healthy, the deploy script removes unused old GHCR images for this app.
...
- [x] Remove unused old app images after a healthy deployment.
