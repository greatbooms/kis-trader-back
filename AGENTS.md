...
6. 공통 에러 처리/Apollo 유틸은 `src/lib/`에 둘 수 있음 (예: `client/src/lib/apollo-utils.ts`)
...
**현재 구조 반영 현황:**
- `src/trading/dto/`
  - `strategy-info.object.ts`
  - `strategy-allocation.object.ts`
  - `market-regime.object.ts`
  - `risk-state.object.ts`
  - `set-strategy-allocation.input.ts`
- `src/trade-record/dto/`
  - `trade-record.object.ts`
  - `position.object.ts`
  - `stock-price.object.ts`
  - `dashboard-summary.object.ts`
- `src/auth/dto/auth.object.ts`
- `src/notification/types/notification.types.ts`
- `src/trading/types/trading-strategy.type.ts`

**현재 예외 / 추후 정리 권장:**
- `src/watch-stock/watch-stock.resolver.ts`
  - `WatchStockType`
  - `CreateWatchStockInput`
  - `UpdateWatchStockInput`
  - 위 3개는 resolver 내부에 남아 있으므로, 해당 파일을 다시 크게 수정할 때 `src/watch-stock/dto/`로 분리 권장
