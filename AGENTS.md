...
## Module Documentation (AGENTS.md per module)

각 모듈 디렉토리(`src/{module}/`)에는 **`AGENTS.md`가 있어야 한다**. 모듈 분리/신설 시 함께 작성.
...
- **루트 `AGENTS.md`(이 파일)**: 프로젝트 전체 규칙
- **모듈 `AGENTS.md`**: 모듈 특화 규칙/주의사항
...
### 4. 결정 문서화
- 비자명한 선택(예: "왜 일일 상한을 3×로?")은 **모듈 AGENTS.md**에 근거 남기기
