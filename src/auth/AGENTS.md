...
- `auth.service.ts` — 로그인 검증, JWT 발급, **DB 기반** clientKey 단위 brute-force 방어 (`LoginAttempt` 테이블, 15분 슬라이딩 윈도우, 5회 실패 시 15분 차단)
...
- `@prisma/client` — `LoginAttempt` 테이블 (brute-force 방어 영속화)
...
- `LoginAttempt` 테이블은 clientKey unique. 다중 인스턴스 / 재시작 후에도 차단 상태 유지. 윈도우 만료 시 `assertLoginAllowed`가 자동 정리
