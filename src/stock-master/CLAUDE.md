...
- **`StockMasterService` export만**: resolver는 같은 모듈 내부용. 다른 모듈은 service의 `searchStocks` / `getStocksByExchange` / `getStats` 메서드 직접 사용 (예: `ScreeningCandidateCollector`가 종목명 fallback에 사용)
