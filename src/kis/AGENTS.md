...
- **resolver/strategy에서 `axios` 직접 호출 금지**: 모든 KIS API는 `KisDomesticService`/`KisOverseasService` 경유 (루트 AGENTS.md "Infrastructure는 격리")
...
- **수정주가 일관성**: 백테스트/과거 데이터는 항상 수정주가 기준 (`MODP=1`/`FID_ORG_ADJ_PRC=0`) — 루트 AGENTS.md
