...
- **데이트레이드 스크리닝(`day-trade-fast`)의 비자명한 규칙**:
  ...
  - 임계값 근거: 평균 거래대금 ≥ 300억(시장가 슬리피지 무시 수준), ATR14% ≥ 1.2(왕복 비용 ~0.3% 대비 4배), 전일 종가 > MA20(백테스트 2023-06~2026-05에서 MA20 위 레짐만 양의 엣지). momentum-breakout 백테스트 결론(거래세 면제 ETF만 양의 기대값)이 전체 설계의 근거 — `src/backtest/AGENTS.md` 참조
