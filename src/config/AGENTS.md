...
- 코드 다른 곳에서 `process.env` 직접 접근 금지 — 반드시 `configService.get<T>('namespace.key')` 경유 (루트 `AGENTS.md` 원칙)
...
- 새 환경 변수 추가 시: `configuration.ts` 매핑 + `.env.example`(있다면) 갱신 + 모듈 `AGENTS.md` 메모
