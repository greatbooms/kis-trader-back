...
- **SELL 승인 분류는 명시적 allowlist**: `stop-loss`/`intraday-stop`, `risk-liquidation`, `eod-exit`, `carryover-exit`, `trailing-stop` phase와 레거시 보호성 reason만 승인 대상으로 본다. 추세소멸·과열청산·일반 익절·알 수 없는 일반 SELL은 자동 실행을 유지하고, `infinite-buy` 익절만 T가 20 이상일 때 추가로 승인 대기한다.
