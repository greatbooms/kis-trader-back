...
- `trading-broker-order-submission.service.ts` — 국내/해외·BUY/SELL broker 주문 mutation dispatch의 단일 gateway
...
- `trading-account-cash-sync.service.ts` — broker 예수금 조회와 broker×market `account_status_cache`의 원자적 갱신
- `trading-order-reconciliation.service.ts` — 미체결 주문 상태 추적, 취소/체결 확정 + 전략별 후처리(carry 리셋 등)
- `trading-broker-order-matcher.service.ts` — 저장된 3-tuple broker context를 검증하고 record의 명시적 broker 주문 이력에서 불명 주문 복구 후보만 보수적으로 필터링하는 GET-only 서비스
- `trading-broker-order-resolution.service.ts` — broker-scoped 불명 주문 후보의 기존 기록 충돌 검사, 사용자 선택 후보 연결, 미주문/기존 기록 일치 확정을 CAS/감사 트랜잭션으로 처리하는 서비스. 모든 확인은 mutation 시점에 record broker를 완전히 재조회한 뒤 수행하며 broker POST는 호출하지 않음
- `trading-broker-cancellation-recovery.service.ts` — 취소 결과 불명 주문을 record broker의 완전한 체결/미체결 GET으로 재검증하고, 확인 가능한 종료 또는 미접수 상태만 CAS/감사 트랜잭션으로 확정하는 서비스
- `trading-broker-order-recovery.resolver.ts` — 웹 인증 사용자와 복구 서비스 사이의 얇은 GraphQL 어댑터. 15초 목록 조회는 DB-only이며 broker 조회는 명시적 mutation에서만 수행
...
- **체결 후 예수금 동기화**: `OrderSyncService`는 reconciliation에서 새 체결이 확인된 경우에만 `TradingAccountCashSyncService.refreshMarketCash(broker, market)`를 호출한다. broker/캐시 실패는 체결 확정을 되돌리지 않는다. 캐시는 schema migration 없이 `account_status_cache:{broker}:{market}` AppSetting row와 같은 문자열의 advisory transaction lock을 사용하며, legacy `account_status_cache` row는 scoped row가 없을 때만 KIS로 해석한다. 성공한 빈 market snapshot도 빈 scoped row로 저장해 legacy fallback을 차단하고, 조회 실패 market은 기존 row를 보존한다. KIS refresh는 Phase 2 바이너리가 더 이상 rollback target이 아니게 될 때까지 scoped row와 legacy row를 한 트랜잭션에서 dual-write하며, 그 시점 이후 legacy write를 제거한다.
- 체결 알림의 종목 합산 보유와 일일요약의 크로스 브로커 노출은 실제 `Position`에서 도출하며, 동일 종목을 2개 이상 broker가 보유할 때만 전달한다. 활성 broker 설정 목록은 표시 판단에 사용하지 않는다.
- **체결 알림 보유 수치의 신선도**: `notifyTradeFill`의 `보유 현황`/`전 증권사 합산`은 모두 마지막 포트폴리오 동기화 시점의 `Position` 스냅샷에서 읽는다 (같은 쿼리·같은 행). `Position`은 `syncPositions`(브로커 잔고)만 갱신하고 `applyReconciledStrategyFill`은 갱신하지 않으므로, 포트폴리오 sync를 선행하지 않는 `executeBrokerMarket` 경로에서는 방금 체결이 아직 반영되지 않았을 수 있다. 알림 경로에서 브로커 잔고를 재조회하지 않는 이유는 rate limit — 대신 호출 경로가 sync를 선행하도록 맞추는 것이 해법이다 (`syncMarketOrdersOnly`/`syncLatestMarketStateBeforeDailySummary`는 이미 선행한다).
- **broker reconciliation 경계**: scheduled order sync는 active broker별 주문/포지션/record/context snapshot을 분리한다. reconciliation과 자동 미체결 취소는 broker를 필수 인자로 전달하고 `{broker, environment, accountHash}` 전체를 재검증하며, broker별 API 실패는 다른 broker loop를 중단하지 않는다. 같은 orderNo/환경/hash가 충돌해도 다른 broker record를 조회·취소하지 않는다.
...
- **In-memory 상태**:
  - `TradingOrchestrator.runningBrokerMarkets` — broker×market 실행 단위 중복 방지 mutex
  - `TradingOrchestrator.lastRiskAlertDate` — broker×market 일별 리스크 알림 중복 방지 (날짜 단위)
...
- **SELL 승인 결정/제출 원자성**: `TradingSellApprovalWorkflowService`만 `PENDING/AWAITING_APPROVAL` pair를 한 트랜잭션에서 `APPROVED/SUBMITTING` 또는 `REJECTED/CANCELLED`로 claim한다. 승인자는 `slack.approverUserIds` exact allowlist로 DB/broker 접근 전에 검증한다. APPROVE는 record의 명시적 broker와 저장된 environment/account hash 전체를 pair claim 전·잔고 refresh 전·`submissionStartedAt` CAS 직전·CAS 직후·감사 로그 후 broker 호출 직전에 현재 3-tuple context와 비교하고, 누락/불일치/도중 전환이면 fail-closed한다. CAS 뒤 전환은 자신이 획득한 정확한 `submissionStartedAt`으로만 `CANCELLED` 전환하며, REJECT는 계좌 context와 무관하게 기존 pair를 취소할 수 있다. claim winner만 record broker 잔고를 새로 읽고 venue/stock holding을 매칭해 수량을 clamp한 뒤 `submissionStartedAt` CAS winner로서 감사 로그를 남기고 SELL을 한 번 호출한다. pre-claim·pre-POST live switch 중 하나라도 꺼지면 제출하지 않으며, `UNKNOWN`/throw/불완전 ACCEPTED identity는 `SUBMISSION_UNKNOWN` recovery로 보낸다. ACCEPTED 뒤에는 broker를 재호출하지 않고 DB 저장만 최대 총 3회 시도한다. Slack 버튼 업데이트는 항상 DB transaction 밖의 best-effort이며, 미제출 표시는 거래가 `CANCELLED`로 확정된 경우에만 사용한다.
...
- **불명 주문/취소 복구 경계**: `TradingBrokerOrderRecoveryService`가 Slack/웹 공용 public API와 DB-only 큐를 소유한다. 제출 후보 연결·미제출·기존 기록 일치는 `TradingBrokerOrderResolutionService`, 취소 불명 상태는 `TradingBrokerCancellationRecoveryService`에 위임한다. 상태 변경 전에는 현재 broker context와 record broker의 완전한 GET 결과를 다시 검증하고 CAS와 감사 로그를 한 트랜잭션으로 처리한다. 양쪽 목록에서 주문이 보이지 않는다는 사실만으로 종료를 추론하지 않으며, 어떤 복구 액션도 broker POST/취소 재시도를 호출하지 않는다.
- **자동 미체결 취소 TOCTOU 경계**: `TradingOrderCancellationService`는 명시적 broker와 미체결 식별자를 fail-closed 정규화하고 현재 `{broker, environment, accountHash}`를 포함한 전체 주문 tuple로 로컬 open record를 조회한다. cancellation claim 뒤 broker predicate를 포함해 record를 재조회하고 `SUBMITTING`과 최초 tuple/context가 그대로인지 확인한 뒤, 해당 broker POST 직전에 live switch와 현재 3-tuple context를 다시 확인한다. 어느 검증이든 실패하면 claim을 해제하며 취소 POST는 호출하지 않는다.
...
- **broker context preview binding**: 복구 목록 polling은 현재 broker context를 조회하거나 계정 정보를 보강하지 않고 item에 record broker만 노출한다. legacy context 배정 확인창을 열 때는 명시적 broker를 받는 별도 인증 query로 환경·마스킹 계좌와 `{broker, environment, accountHash}`에 HMAC으로 묶인 opaque token을 발급한다. 배정 mutation은 먼저 target record의 broker를 읽고 같은 broker token인지 확인한 뒤 broker predicate를 포함한 기존 CAS를 수행한다. 웹과 Slack은 확인 시 같은 token을 제출해야 하며, broker/계좌/환경이 바뀌면 fail-closed한다. hash/raw 계좌와 token은 로그·감사 details에 남기지 않는다.
